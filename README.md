...
- `exact_mismatch` reporting is byte-exact for `strict` rows and shape-semantic for `coverage` rows (median-thresholded inside mask, with up to 1 edge pixel tolerance), so channel-layout differences and tie-break jitter do not inflate nightly residue.
